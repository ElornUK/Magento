...
class Process extends Action implements HttpPostActionInterface, HttpGetActionInterface, CsrfAwareActionInterface {
	...
	public function execute() {
        try {
			$orders = $this->_orderFactory->create()->loadByIncrementId($this->checkoutSession->getLastRealOrderId());
            ...
            // TODO: change generic error message to something more specific
            ...
            // Can these next 2 lines be safely removed?
			$orders->addStatusToHistory('canceled', 'Something went wrong with the payment, we were not able to process it, please contact support.', false);
            $orders->save();
